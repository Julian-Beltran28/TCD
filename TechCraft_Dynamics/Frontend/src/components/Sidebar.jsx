...
// ✅ Función para normalizar roles (igual que en App.jsx)
...
export default function Sidebar({ isOpen }) {
  ...
  const normalizedUserRole = normalizeRole(user?.rol) || 'admin';
  ...
  const menuItems = [
    // Perfil
    {
      label: 'Perfil',
      path: 'perfil',
      roles: ['admin', 'supervisor', 'staff'], // ✅ Permitido para los 3 roles
      icon: (
    ...
    // Ventas
    {
      label: 'Ventas',
      path: 'ventas',
      roles: ['admin', 'supervisor', 'staff'], // ✅ Permitido para 3 roles
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24" fill="#e8eaed">
    ...
    // Usuarios
    {
      label: 'Usuarios',
      path: 'usuarios',
      roles: ['admin', 'supervisor'],  // ✅ Permitido para 2 roles
      icon: (
    ...
    // Reportes
    {
      label: 'Reportes',
      path: 'reportes',
      roles: ['admin','supervisor', 'staff'], // ✅ Permitido para 2 roles
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24" fill="#e8eaed">
    ...
    <div className={`sidebar ${isOpen ? 'open' : 'collapsed'}`}>
      <div className="sidebar-content">
        ...
        <ul className="nav flex-column mt-3">
          {menuItems
            .filter(item => item.roles.includes(normalizedUserRole)) // ✅ Usar rol normalizado
            .map((item, index) => (
